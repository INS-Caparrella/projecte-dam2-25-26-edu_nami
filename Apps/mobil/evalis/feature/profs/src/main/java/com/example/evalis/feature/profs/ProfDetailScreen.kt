...
import com.example.evalis.core.domain.AssigAlumne
...
fun ProfsDetail(dni: String, profId: String, onClose: () -> Unit = {}) {
    ...
    // EvalisTheme eliminado — el tema ya se aplica desde MainActivity
...
private fun parsejarProf(obj: JSONObject): ParsedProf {
    ...
    for (i in 0 until assignaturesArr.length()) {
        ...
        val nomAssig = t.optString("nom_assignatura")
        val aula = t.optString("aula")
        if (nomAssig.isNotBlank()) {
            assignatures.add(AssigAlumne(nomAssig, "Aula: $aula"))
        }
    }

    return ParsedProf(fullName, email, fotoUrl, assignatures)
}
